...
vec3 mai_voice_embodiment_sampleGradient(vec2 position) {
    ...
    // Unrolled loop for all 15 ellipses
    // Ellipse 0
    float value0 = mai_voice_embodiment_ellipseValue(position, ellipses[0].center, ellipses[0].size, ellipses[0].blur);
    if (ellipses[0].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[0].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value0);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[0].color, value0, result, 1.0);
    }

    // Ellipse 1
    float value1 = mai_voice_embodiment_ellipseValue(position, ellipses[1].center, ellipses[1].size, ellipses[1].blur);
    if (ellipses[1].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[1].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value1);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[1].color, value1, result, 1.0);
    }

    // Ellipse 2
    float value2 = mai_voice_embodiment_ellipseValue(position, ellipses[2].center, ellipses[2].size, ellipses[2].blur);
    if (ellipses[2].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[2].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value2);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[2].color, value2, result, 1.0);
    }

    // Ellipse 3
    float value3 = mai_voice_embodiment_ellipseValue(position, ellipses[3].center, ellipses[3].size, ellipses[3].blur);
    if (ellipses[3].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[3].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value3);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[3].color, value3, result, 1.0);
    }

    // Ellipse 4
    float value4 = mai_voice_embodiment_ellipseValue(position, ellipses[4].center, ellipses[4].size, ellipses[4].blur);
    if (ellipses[4].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[4].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value4);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[4].color, value4, result, 1.0);
    }

    // Ellipse 5
    float value5 = mai_voice_embodiment_ellipseValue(position, ellipses[5].center, ellipses[5].size, ellipses[5].blur);
    if (ellipses[5].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[5].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value5);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[5].color, value5, result, 1.0);
    }

    // Ellipse 6
    float value6 = mai_voice_embodiment_ellipseValue(position, ellipses[6].center, ellipses[6].size, ellipses[6].blur);
    if (ellipses[6].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[6].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value6);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[6].color, value6, result, 1.0);
    }

    // Ellipse 7
    float value7 = mai_voice_embodiment_ellipseValue(position, ellipses[7].center, ellipses[7].size, ellipses[7].blur);
    if (ellipses[7].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[7].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value7);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[7].color, value7, result, 1.0);
    }

    // Ellipse 8
    float value8 = mai_voice_embodiment_ellipseValue(position, ellipses[8].center, ellipses[8].size, ellipses[8].blur);
    if (ellipses[8].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[8].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value8);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[8].color, value8, result, 1.0);
    }

    // Ellipse 9
    float value9 = mai_voice_embodiment_ellipseValue(position, ellipses[9].center, ellipses[9].size, ellipses[9].blur);
    if (ellipses[9].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[9].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value9);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[9].color, value9, result, 1.0);
    }

    // Ellipse 10
    float value10 = mai_voice_embodiment_ellipseValue(position, ellipses[10].center, ellipses[10].size, ellipses[10].blur);
    if (ellipses[10].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[10].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value10);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[10].color, value10, result, 1.0);
    }

    // Ellipse 11
    float value11 = mai_voice_embodiment_ellipseValue(position, ellipses[11].center, ellipses[11].size, ellipses[11].blur);
    if (ellipses[11].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[11].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value11);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[11].color, value11, result, 1.0);
    }

    // Ellipse 12
    float value12 = mai_voice_embodiment_ellipseValue(position, ellipses[12].center, ellipses[12].size, ellipses[12].blur);
    if (ellipses[12].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[12].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value12);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[12].color, value12, result, 1.0);
    }

    // Ellipse 13
    float value13 = mai_voice_embodiment_ellipseValue(position, ellipses[13].center, ellipses[13].size, ellipses[13].blur);
    if (ellipses[13].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[13].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value13);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[13].color, value13, result, 1.0);
    }

    // Ellipse 14
    float value14 = mai_voice_embodiment_ellipseValue(position, ellipses[14].center, ellipses[14].size, ellipses[14].blur);
    if (ellipses[14].isScreen) {
        vec3 screenBlend = vec3(1.0) - (vec3(1.0) - ellipses[14].color) * (vec3(1.0) - result);
        result = mix(result, screenBlend, value14);
    } else {
        result = mai_voice_embodiment_compositeOver(ellipses[14].color, value14, result, 1.0);
    }
...
MAIVoiceEmbodimentBlobShapeInfo mai_voice_embodiment_calculateBlobShape(vec2 uv, float radius, float time, float distortionIntensity,
    ...
    float shadowWidth = max(aaWidth, blurRadius * 1.5 + extraEdgeBlur);
...
void main() {
    // ALWAYS render background first if we have one, filling the entire screen
    vec4 finalColor;
    if (u_BackgroundAlpha > 0.0) {
        // Set background color for the entire screen, using original position
        // This ensures background is not affected by container scale
        finalColor = vec4(u_BackgroundColor, u_BackgroundAlpha);
    }

    // For content, we'll use the scaled position
    vec2 scaledPosition = v_Position * (1.0 / u_ContainerScale);

    // Calculate UV coordinates for content using scaled position
    vec2 contentUV = scaledPosition * 0.5 + 0.5;
    ...
    if (u_ActiveBlob != 0) {
        ...
        if (u_BackgroundAlpha > 0.0) {
            // Composite blob over the background
            vec3 contentColor = blobColor.rgb * mask;
            finalColor.rgb = contentColor + finalColor.rgb * (1.0 - mask);
            finalColor.a = u_BackgroundAlpha + mask * (1.0 - u_BackgroundAlpha);
    ...
    if (u_BackgroundAlpha > 0.0) {
        // Composite final content over the background
        finalColor.rgb = compColor * outAlpha + finalColor.rgb * (1.0 - outAlpha);
        finalColor.a = u_BackgroundAlpha + outAlpha * (1.0 - u_BackgroundAlpha);
        FragColor = finalColor;
    } else {
